...
File Description: Main model file version 6 (with categoric input for virulence strengths L,M,H)
"""
#%%
# load packages
...
# AGENT INITIALIZATION     
    ...
    def __init__(self, model, microbe_id, species, virulence_factors):#, interactions, interactions_env):
        ...
        self.vf_phag_cellbound = self.virulence_factors[0]
        self.vf_phag_secreted = self.virulence_factors[1]
        self.phag_prob_M_original = self.species[4] - self.vf_phag_cellbound # original baseline phagocytose probability
...
#                self.model.number_microbes_grid[self.pos[0]][self.pos[1]] +=1 # add counter at new position            
    ...
    def virulence_act(self):
        ...
        for microbe in microbe_cellmates:
            if microbe.alive and microbe.vf_phag_secreted != 0: # if the microbe cellmate excretes virulence molecules 
                if self.phag_prob_M - microbe.vf_phag_secreted > 0: # phagocytose probability can't be smaller than 0
                    self.phag_prob_M -= microbe.vf_phag_secreted 
...
# MODEL INITIALIZATION
    ...
    def __init__(self, microbe_n, microbe_capacity, all_species, all_species_virulence, virulence_strength_values, # microbes
...
#        self.number_microbes_grid = [[0 for j in range(self.gridrange)] for i in range(self.gridrange)] # counts agents per patch
        ...
        # virulence
        self.all_species_virulence = copy.deepcopy(all_species_virulence)
        self.virulence_strength_values = virulence_strength_values
        self.set_virulence() # sets model level recruitment virulence and inserts values of virulence strength instead of 'none', 'low', 'medium', 'high'
    ...
    # CREATE MICROBES
        ...
        for i in range(self.number_species):
            self.create_microbes(ns[i], all_species[i], self.all_species_virulence[i])#, [x[i] for x in all_interactions], [x[i] for x in all_interactions_env]) # create chosen number of each type of species
...
# FUNCTIONS FOR MODEL INITIALIZATION
    ...
    def set_virulence(self):
        for species_virulence in self.all_species_virulence:
            for i, virulence in enumerate(species_virulence):
                species_virulence[i] = self.virulence_strength_values[i][virulence]

        self.vf_recruit_species = dict(zip((self.species_names),(1-self.all_species_virulence[i][2] for i in range(len(self.all_species_virulence))))) # dict of species names and recruitment virulence effects (vf_recruit)        
