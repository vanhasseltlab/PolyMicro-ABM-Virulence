...
File Description: Simulation script for a dualspecies population. Species A posseses 3 virulence factors (vc, vs and vr) and species B posseses 3 virulence factors (vc, vs and vr).
"""
...
virulence_types_A = ['cellbound','secreted','recruit']
virulence_types_B = ['cellbound','secreted','recruit']
all_species_vts = [virulence_types_A] + [virulence_types_B]

# create list per species of indeces of virulence types
species_vts_i = [[],[]] 
for (i,species_vts),(j,vt) in [((i,species_vts),(j,vt)) for i,species_vts in enumerate(all_species_vts) for j,vt in enumerate(virulence_types)]:
    if (vt in species_vts):
        species_vts_i[i].append(j) 

# create lists for species of low, medium and high strength
all_species_all_strengths = [[],[]]
for vs in virulence_strengths:
    for i, species in enumerate(species_vts_i):
        strength = ["None", "None", "None"]
        for vt_i in species:
            strength[vt_i] = vs
        all_species_all_strengths[i].append(strength)
    
all_species_all_virulence = []
for species_A_strength in all_species_all_strengths[0]:
    for species_B_strength in all_species_all_strengths[1]:
        all_species_all_virulence.append([species_A_strength, species_B_strength])
...
#ab_conc_range_SI = [[x] for x in [x*species_MIC[0] for x in [1,1.15,1.4,4]]]
...
data_collection = 30 # interval for datacollection in mins
...
os.chdir('/tmpres/eske')
folder_name = "/220705_10_Acsr_Bcsr"  
